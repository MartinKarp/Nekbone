...
__kernel void ax(__global double * restrict w,
    ...
    for(unsigned ele = 0; ele < N; ele += LX1*LY1*LZ1){
        ...
        double shu1[LX1*LY1*LZ1];
        double shu2[LX1*LY1*LZ1];
        double shu3[LX1*LY1*LZ1];
            ...
            for(unsigned j = 0; j < LY1; j++){
                ...
                for(unsigned i = 0; i < LX1; i++){
                    ...
                    for (unsigned l = 0; l<LX1; l++){
                      rtmp += shdxtm1[l+i*LX1] * shu1[l+j*LX1 +k*LX1*LY1];
                      stmp += shdxtm1[l+j*LX1] * shu2[i+l*LX1 + k*LX1*LY1];
                      ttmp += shdxtm1[l+k*LX1] * shu3[ij + l*LX1*LY1];
                    }
                    shur[ijk] = G00*rtmp
                             + G01*stmp
                             + G02*ttmp;
