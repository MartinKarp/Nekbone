...
c   Target necessary for OpenCL
      parameter(npd = 2)
      parameter (npad = (lx1+npd)*(ly1+npd)*(lz1+npd)*lelt)
      parameter (nxnpdn = (lx1+npd)*(ly1+npd))
      real, target :: x(n),f(n),r(n),w(n),p(n),z(n),g(6*n),c(n),
     $ dxm1,dxtm1, g7(n),g1(npad), g2(npad), g3(npad), g4(npad), 
     $ g5(npad),g6(npad),p2(npad),w2(npad),
     $ dxnpd(nxnpdn),dxtnpd(nxnpdn)
!      real x(n),f(n),r(n),w(n),p(n),z(n),g(1),c(n)
...
c OPencl things
     ...
     $ platform_ids(:), device_ids(:)
      ...
      idevice = 1
      iplatform = 2
      byte_size= 8_8 * int(npad,8)
      g_size= 8_8 * int(6*npad,8)
      element_size = 8_8*int(nxnpdn,8)
      npd2 = npad
...
c Init arrays
     ...
     $      CL_MEM_READ_WRITE,element_size,C_NULL_PTR, err)
      ...
      if (err.ne.0) stop 'clSetKernelArg'
      err=clSetKernelArg(ax_kernel,10,sizeof(8_4),C_LOC(npd2))
      if (err.ne.0) stop 'clSetKernelArg'
      ...
      print *, 'before g' 
      call copy(g7,g1,n)
      call padv(g7,g1,lx1,lelt,npd)
      call copy(g7,g2,n)
      call padv(g7,g2,lx1,lelt,npd)
      call copy(g7,g3,n)
      call padv(g7,g3,lx1,lelt,npd)
      call copy(g7,g4,n)
      call padv(g7,g4,lx1,lelt,npd)
      call copy(g7,g5,n)
      call padv(g7,g5,lx1,lelt,npd)
      call copy(g7,g6,n)
      call padv(g7,g6,lx1,lelt,npd)
      pap = 0.0
c     set machine tolerances
      one = 1.
      eps = 1.e-20
     ...
     $    byte_size,C_LOC(g6), 0,C_NULL_PTR,C_NULL_PTR)
      ...
      call paddx(dxm1,dxnpd,lx1,npd)
      call paddx(dxtm1,dxtnpd,lx1,npd)
      err=clEnqueueWriteBuffer(cmd_queue,cl_dxm1,CL_TRUE,0_8,
     $    element_size,C_LOC(dxnpd), 0,C_NULL_PTR,C_NULL_PTR)
      if (err.ne.0) stop 'clEnqueueWriteBuffer'
      err=clEnqueueWriteBuffer(cmd_queue,cl_dxtm1,CL_TRUE,0_8,
     $    element_size,C_LOC(dxtnpd), 0,C_NULL_PTR,C_NULL_PTR)
      if (err.ne.0) stop 'clEnqueueWriteBuffer'
c     call tester(z,r,n)  
      do iter=1,miter
         call solveM(z,r,n)    ! preconditioner here
...
!         if (err.ne.0) stop 'clEnqueueEnqueueNDRange'
     ...
     $      0_8,byte_size,C_LOC(w2),0,C_NULL_PTR,C_NULL_PTR)
         if (err.ne.0) stop 'clEnqueueReadBuffer'
         !print *, g(n*6-4:n*6) 
         err=clFinish(cmd_queue)
         if (err.ne.0) stop 'clFinish'
         call unpad(w,w2,lx1,lelt,npd)
...
c     ---------------------------------------------------------------
      ...
      subroutine padv(x,xp,nx,nel,p)
      integer nx,nel,p
      real x(nx,nx,nx,nel), xp(nx+p,nx+p,nx+p,nel)
      integer e,k,j,i
      xp = 0.0
      do e=1,nel
      do k=1,nx
...
c     ---------------------------------------------------------------
      ...
      subroutine unpad(x,xp,nx,nel,p)
      integer nx,nel,p
      real x(nx,nx,nx,nel), xp(nx+p,nx+p,nx+p,nel)
      integer e,k,j,i
...
c     ---------------------------------------------------------------
      ...
      subroutine paddx(x,xp,nx,p)
      integer nx,nel,p
      real x(nx,nx), xp(nx+p,nx+p)
      integer e,k,j,i
      xp = 0.0 
      do j=1,nx
      do i=1,nx
