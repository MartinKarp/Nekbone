...
__kernel void ax(__global double * restrict w,
    ...
    for(unsigned ele = 0; ele < N; ele += LX1*LY1*LZ1){
        ...
        for (unsigned k=0; k<LZ1; ++k){
		    ...
		    double wijke = 0.0;
                    for(unsigned l = 0; l<LX1; l++){
                        shft_r[30] =shft_r[0] + shdxm1[l + i*LX1] * shur[l+j*LX1+k*LX1*LY1];
                        shft_s[30] =shft_s[0] + shdxm1[l + j*LX1] * shus[i+l*LX1+k*LX1*LY1];
                        shft_t[30] =shft_t[0] + shdxm1[l + k*LX1] * shut[i+j*LX1+l*LX1*LY1];
                        #pragma unroll
                        for(unsigned l2 = 0; l2 < 30; l2++){
                            shft_r[l2] = shft_r[l2 + 1];  
                            shft_s[l2] = shft_s[l2 + 1];  
                    ...
                    #pragma unroll
		    for (unsigned l = 0; l<31; l++){
                        wijke += shft_r[l] + shft_s[l] + shft_t[l];
                    }
                    shw[ijk] = wijke;
                }
            }
