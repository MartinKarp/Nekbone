...
c OPencl things
      ...
      integer(c_intptr_t), target :: 
     $ cmd_queue,cmd_queue2,prog,context,
     $ ax_kernel,read_kernel,binary_status
     ...
     $                           device_ids, context, cmd_queue)
      cmd_queue2 = clCreateCommandQueue(context, 
     $                device_ids(idevice), 0_8, err)
     ...
     $ 0, C_NULL_PTR,C_NULL_PTR,C_NULL_FUNPTR,C_NULL_PTR)
      if (err.ne.0) stop 'clBuildProgram'
      ...
      enddo
      c_kernel_name(irec+1)=C_NULL_CHAR
      read_kernel=clCreateKernel(prog,C_LOC(c_kernel_name),err)
      if (err.ne.0) stop 'clCreateKernel'
...
!      if (err.ne.0) stop 'clCreateBuffer'
     ...
     $      byte_size,C_NULL_PTR, err)
      ...
      if (err.ne.0) stop 'clSetKernelArg'
      err=clSetKernelArg(ax_kernel,2,sizeof(8_8),C_LOC(cl_g2))
      if (err.ne.0) stop 'clSetKernelArg'
...
!      if (err.ne.0) stop 'clSetKernelArg'
      err=clSetKernelArg(read_kernel,0,sizeof(8_8),C_LOC(cl_g))
      if (err.ne.0) stop 'clSetKernelArg'
      err=clSetKernelArg(read_kernel,1,sizeof(8_4),C_LOC(n))
      if (err.ne.0) stop 'clSetKernelArg'
...
c     set machine tolerances
     ...
     $    byte_size,C_LOC(p), 0,C_NULL_PTR,C_NULL_PTR)
      if (err.ne.0) stop 'clEnqueueWriteBuffer'
      err=clEnqueueWriteBuffer(cmd_queue2,cl_g,CL_TRUE,0_8,
...
c     call tester(z,r,n)  
      do iter=1,miter
         ...
         !if (iter.eq.1) beta=0.0
         !call add2s1(p,z,beta,n)                                         ! 2n
         err=clEnqueueTask(cmd_queue2,read_kernel,
...
!      if (nid.eq.0) write(6,6) iter,rnorm,alpha,beta,pap
      ...
      if (err.ne.0) stop 'clReleaseCommandQueue'
      err = clReleaseCommandQueue(cmd_queue2) 
      if (err.ne.0) stop 'clReleaseCommandQueue'
      err = clReleaseContext(context)  
      if (err.ne.0) stop 'clReleaseContext'
      print *,'released'
      return
      end
c      ---------------------------------------------------------------
